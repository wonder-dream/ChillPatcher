...
namespace ChillPatcher.UIFramework.Music
{
    ...
    public class PlayQueueManager
    {
        ...
        /// <summary>
        /// 批量移除指定 UUID 的歌曲（从队列和历史中移除）
        /// 如果当前正在播放的歌曲也在列表中，会跳过当前播放的，但会清除历史记录
        /// </summary>
        /// <param name="uuids">要移除的 UUID 集合</param>
        /// <returns>移除的歌曲数量</returns>
        public int RemoveByUUIDs(HashSet<string> uuids)
        {
            if (uuids == null || uuids.Count == 0) return 0;
            ...
            for (int i = _queue.Count - 1; i >= 1; i--)
            {
                if (uuids.Contains(_queue[i].UUID))
                {
                    Plugin.Log.LogDebug($"[Queue] Removing from queue: {_queue[i].AudioClipName}");
                    _queue.RemoveAt(i);
                    removedCount++;
                }
            }
            ...
            // 从历史记录移除
            int historyRemoved = _history.RemoveAll(a => uuids.Contains(a.UUID));
            removedCount += historyRemoved;
            ...
            if (removedCount > 0)
            {
                ...
                OnQueueChanged?.Invoke();
                OnHistoryChanged?.Invoke();
                Plugin.Log.LogInfo($"[Queue] Removed {removedCount} songs by UUID set (queue: {removedCount - historyRemoved}, history: {historyRemoved})");
            }
